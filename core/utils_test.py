...
class UtilsTests(test_utils.GenericTestBase):
    ...
    def test_require_valid_url_fragment(self) -> None:
        ...
        with self.assertRaisesRegex(Exception, name_with_spaces_expected_error):
            utils.require_valid_url_fragment(name_with_spaces, 'name-type', 20)
        ...
        with self.assertRaisesRegex(Exception, name_in_caps_expected_error):
            utils.require_valid_url_fragment(name_in_caps, 'name-type', 20)
    ...
    def test_get_human_readable_time_string(self) -> None:
        ...
        with self.assertRaisesRegex(AssertionError, 'Time cannot be negative'):
    ...
    def test_convert_image_binary_to_data_url(self) -> None:
        ...
        self.assertEqual(
            utils.convert_image_binary_to_data_url(file_contents_png, 'png'),
            'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAcAAAAGCAIAAACAbBMhAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAAEnQAABJ0Ad5mH3gAAAAySURBVBhXY/iPDYBEV6xY0draCuFDAEgUKMTAANUEUYFuAkQFihIIGwigosiG/P//HwD5HmjphyAmJQAAAABJRU5ErkJggg%3D%3D',
        )  # pylint: disable=line-too-long
        self.assertEqual(
            utils.convert_image_binary_to_data_url(file_contents_webp, 'webp'),
            'data:image/webp;base64,UklGRlIAAABXRUJQVlA4IEYAAADQAQCdASoHAAYAAgA0JaQAAv%2B5x9YuAAD%2B%2B0nD9oP5zmavp/Nyl8%2Bf/REL9weER482Ugrc/6dmq28Kx1pj/se/CsMAAAAA',
        )  # pylint: disable=line-too-long

    def test_raise_error_invalid_convert_webp_binary_to_data_url(self) -> None:
        file_contents_png, _ = self._get_png_and_webp_image()
        with self.assertRaisesRegex(
            Exception,
            'The given binary string does not represent a webp image.',
        ):
            utils.convert_image_binary_to_data_url(file_contents_png, 'webp')

    def test_get_exploration_components_from_dir_with_yaml_content(
        self,
    ...
    def test_escape_html_function(self) -> None:
        ...
        self.assertEqual(utils.escape_html(html_data), expected_html_data)
